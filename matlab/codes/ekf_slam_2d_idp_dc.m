...
mu_dc = repmat([0.0], (4*N+3), 1);
robMapSigma_dc = zeros(3,4*N);
mapSigma_dc = INF*eye(4*N);
...
%     [mu_dc, sigma_dc] = prediction_step_2d_dc(mu_dc, sigma_dc, data.timestep(t).odometry, sup_data.u_dist_std, sup_data.u_theta_std);
%     mu_dc(1:3) = mu(1:3);
%     sigma_dc(1:3,1:3) = sigma(1:3,1:3);
    ...
    if ~isempty(data.timestep(t).sensor)
        ...
        [mu_dc, sigma_dc, observedLandmarks] = correction_step_2d(mu_dc, sigma_dc, data.timestep(t).sensor, observedLandmarks,init_depth,sup_data.obs_std);
    end
...
for n=1:N
  lm_2d_xy_coord(n,:) = [(1/mu_dc(3+4*(n-1)+4))*cos(mu_dc(3+4*(n-1)+3)) + mu_dc(3+4*(n-1)+1), ...
      (1/mu_dc(3+4*(n-1)+4))*sin(mu_dc(3+4*(n-1)+3)) + mu_dc(3+4*(n-1)+2)];
end
