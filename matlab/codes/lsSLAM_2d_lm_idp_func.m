function [ss_pos_e_rms] = lsSLAM_2d_lm_idp_func(graph_file_name,plot_on)
...
if plot_on
  ...
  figure(1);
  plot_graph_ini_2d(g2d_idp, 0, plot_ini_obs_on);
end
%plot_graph(g, i);
...
gx = g2d_idp.x;
g2d_idp.x = g2d_idp.x_gt;
disp(['ground truth error ' num2str(compute_global_error_2d(g2d_idp))]);

% compute initial error for state vector
g2d_idp.x = gx;
initial_error = compute_global_error_2d(g2d_idp);
...
for i = 1:numIterations
  ...
  % solve the dx
  [dx,H, lm_rau,lm_F_new,lm_lamda] = linearize_and_solve_2d_lm(g2d_idp,lm_F_res,lm_lamda,lm_tau,i);
  ...
  if plot_on
    ...
    figure(2);
    plot_graph_2d(g2d_idp, i,H, plot_obs_on);
  end
...
lm_2d_xy_coord = zeros(g2d_idp.ss_num,2);
for n=1:g2d_idp.ss_num
  lm_2d_xy_coord(n,:) = (g2d_idp.x(g2d_idp.ss_num*2+3*(g2d_idp.lm_ini_pose_correspondance_2d(n,2)-1)+1:g2d_idp.ss_num*2+3*(g2d_idp.lm_ini_pose_correspondance_2d(n,2)-1)+2) + ...
    (1/g2d_idp.x(2*(n-1)+2))*[cos(g2d_idp.x(2*(n-1)+1));sin(g2d_idp.x(2*(n-1)+1))])';
end

lm_2d_xy_coord_gt = zeros(g2d_idp.ss_num,2);
for n=1:g2d_idp.ss_num
  lm_2d_xy_coord_gt(n,:) = (g2d_idp.x_gt(g2d_idp.ss_num*2+3*(g2d_idp.lm_ini_pose_correspondance_2d(n,2)-1)+1:g2d_idp.ss_num*2+3*(g2d_idp.lm_ini_pose_correspondance_2d(n,2)-1)+2) + ...
    (1/g2d_idp.x_gt(2*(n-1)+2))*[cos(g2d_idp.x_gt(2*(n-1)+1));sin(g2d_idp.x_gt(2*(n-1)+1))])';
end
