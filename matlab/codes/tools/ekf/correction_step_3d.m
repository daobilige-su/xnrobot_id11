...
for i = 1:m
	...
	if(observedLandmarks(landmarkId)==false)
        ...
        mu(7+3*(landmarkId-1)+1:7+3*(landmarkId-1)+3) = lm_ini_global(1:3);
    ...
    l_local = [M_x(1:3,1:3)',-M_x(1:3,1:3)'*M_x(1:3,4);0,0,0,1]*[mu(7+3*(landmarkId-1)+1:7+3*(landmarkId-1)+3);1];
    ...
    Hi_low = ekf_3d_jacobian_update(mu(1:7),mu(7+3*(landmarkId-1)+1:7+3*(landmarkId-1)+3));
    %Hi = zeros(2,size(mu,1));
	Hi = zeros(2,size(mu,1));
	Hi(:,1:7) = Hi_low(:,1:7);
	Hi(:,7+3*(landmarkId-1)+1:7+3*(landmarkId-1)+3) = Hi_low(:,8:end);
