function [mu, sigma, observedLandmarks] = correction_step_3d(mu, sigma, z, observedLandmarks,init_depth,obs_azim_std,obs_elev_std)
...
for i = 1:m
	...
	if(observedLandmarks(landmarkId)==false)
        ...
        lm_ini_global = pose3d_quat_to_matrix(mu(1:7))*[init_depth*[cos(z(i).elev)*cos(z(i).azim);cos(z(i).elev)*sin(z(i).azim);sin(z(i).elev)];1];
                
        lm_ini_azim = atan2(lm_ini_global(2)-lm_ini_y,lm_ini_global(1)-lm_ini_x);
        lm_ini_elev = atan2(lm_ini_global(3)-lm_ini_z,sqrt((lm_ini_global(1)-lm_ini_x)^2+(lm_ini_global(2)-lm_ini_y)^2));
        
        lm_ini_idp = 1/init_depth;
        dist_min = 1;
        
        
        mu(7+6*(landmarkId-1)+1:7+6*(landmarkId-1)+6) = [lm_ini_x;lm_ini_y;lm_ini_z;lm_ini_azim;lm_ini_elev;1/(3*dist_min);];
        
        sigma(7+6*(landmarkId-1)+1:7+6*(landmarkId-1)+3,7+6*(landmarkId-1)+1:7+6*(landmarkId-1)+3) = sigma(1:3,1:3);
        sigma(7+6*(landmarkId-1)+4,7+6*(landmarkId-1)+4) = obs_azim_std^2;
        sigma(7+6*(landmarkId-1)+5,7+6*(landmarkId-1)+5) = obs_elev_std^2;
        sigma(7+6*(landmarkId-1)+6,7+6*(landmarkId-1)+6) = (1/(3*dist_min))^2;
    ...
    lm_global_xyz = (1/mu(7+6*(landmarkId-1)+6))*[cos(mu(7+6*(landmarkId-1)+5))*cos(mu(7+6*(landmarkId-1)+4));...
        cos(mu(7+6*(landmarkId-1)+5))*sin(mu(7+6*(landmarkId-1)+4));...
        sin(mu(7+6*(landmarkId-1)+5))]+mu(7+6*(landmarkId-1)+1:7+6*(landmarkId-1)+3);
    
    l_local = [M_x(1:3,1:3)',-M_x(1:3,1:3)'*M_x(1:3,4);0,0,0,1]*[lm_global_xyz;1];
    expectedZ(2*i-1:2*i) = [atan2(l_local(2),l_local(1));atan2(l_local(3),sqrt(l_local(1)^2+l_local(2)^2))];
    ...
    Hi_low = ekf_3d_jacobian_update(mu(1:7),mu(7+6*(landmarkId-1)+1:7+6*(landmarkId-1)+6));
    %Hi = zeros(2,size(mu,1));
	Hi = zeros(2,size(mu,1));
	Hi(:,1:7) = Hi_low(:,1:7);
	Hi(:,7+6*(landmarkId-1)+1:7+6*(landmarkId-1)+6) = Hi_low(:,8:end);
...
Q = diag(repmat([obs_azim_std^2;obs_elev_std^2],m,1));
...
Z_diff = normalize_all_bearings_3d(Z - expectedZ);
