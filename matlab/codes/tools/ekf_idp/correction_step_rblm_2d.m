...
for i = 1:m
    ...
    if(observedLandmarks_rblm(landmarkId)==false)
		...
		mu(N*4+2*landmarkId+2) = mu(1) + z(i).range*cos(z(i).bearing+mu(3));
		mu(N*4+2*landmarkId+3) = mu(2) + z(i).range*sin(z(i).bearing+mu(3));
	...
	expectedZ(2*i-1) = sqrt((mu(N*4+2*landmarkId+2)-mu(1))^2 + (mu(N*4+2*landmarkId+3)-mu(2))^2);
	expectedZ(2*i) = atan2((mu(N*4+2*landmarkId+3)-mu(2)),(mu(N*4+2*landmarkId+2)-mu(1))) - mu(3);
	...
	Hi(:,1:3) = Hi_low(:,1:3);
	Hi(:,N*4+2*landmarkId+2:N*4+2*landmarkId+3) = Hi_low(:,4:5);
