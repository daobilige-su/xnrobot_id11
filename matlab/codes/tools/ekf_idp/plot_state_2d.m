function plot_state_2d(mu, sigma, landmarks, landmarks_rblm, timestep, observedLandmarks, observedLandmarks_rblm, z, z_rblm , robot_pose_hist, robot_pose_gt_hist)
    ...
    for i=1:length(observedLandmarks)
        if(observedLandmarks(i))
            lm_x = (1/mu(3+4*(i-1)+4))*cos(mu(3+4*(i-1)+3)) + mu(3+4*(i-1)+1);
            lm_y = (1/mu(3+4*(i-1)+4))*sin(mu(3+4*(i-1)+3)) + mu(3+4*(i-1)+2);
            plot(lm_x,lm_y, 'bo', 'markersize', 10, 'linewidth', 5)
            
            F = jacobian_idp2euc_2d(mu(3+4*(i-1)+1:3+4*(i-1)+4));
            sigma_lm = F*sigma(3+4*(i-1)+1:3+4*(i-1)+4,3+4*(i-1)+1:3+4*(i-1)+4)*F';
            drawprobellipse([lm_x;lm_y], sigma_lm, 0.997, 'b');
        end
    end

    for i=1:size(z,2)
        mX = (1/mu(3+4*(z(i).id-1)+4))*cos(mu(3+4*(z(i).id-1)+3)) + mu(3+4*(z(i).id-1)+1);
        mY = (1/mu(3+4*(z(i).id-1)+4))*sin(mu(3+4*(z(i).id-1)+3)) + mu(3+4*(z(i).id-1)+2);
