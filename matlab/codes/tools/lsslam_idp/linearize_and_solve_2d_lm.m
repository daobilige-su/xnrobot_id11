...
for eid = 1:length(g.edges)
  ...
  elseif (strcmp(edge.type, 'L') ~= 0)
    ...
    % A Jacobian wrt x1
    % B Jacobian wrt x2
    [e, A, B, C] = linearize_pose_landmark_constraint_2d(x1, x2, edge.measurement,g,edge);
    ...
    anchor_idx = g.ss_num*2+3*(g.lm_ini_pose_correspondance_2d(lm_idx,2)-1)+1;
    
    b(anchor_idx:anchor_idx+2) = (b(anchor_idx:anchor_idx+2)' + (e')*edge.information*C)';
    
    H(edge.fromIdx:edge.fromIdx+1,anchor_idx:anchor_idx+2) = H(edge.fromIdx:edge.fromIdx+1,anchor_idx:anchor_idx+2) + A'*edge.information*C;
    H(anchor_idx:anchor_idx+2,edge.fromIdx:edge.fromIdx+1) = H(anchor_idx:anchor_idx+2,edge.fromIdx:edge.fromIdx+1) + C'*edge.information*A;
    H(edge.toIdx:edge.toIdx+2,anchor_idx:anchor_idx+2) = H(edge.toIdx:edge.toIdx+2,anchor_idx:anchor_idx+2) + B'*edge.information*C;
    H(anchor_idx:anchor_idx+2,edge.toIdx:edge.toIdx+2) = H(anchor_idx:anchor_idx+2,edge.toIdx:edge.toIdx+2) + C'*edge.information*B;
