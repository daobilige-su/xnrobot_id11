...
for eid = 1:length(g.edges)
  ...
  elseif (strcmp(edge.type, 'L') ~= 0)
    ...
    lm_3d_xy_coord = g.x(g.ss_num*3+7*(g.lm_ini_pose_correspondance_3d(lm_idx,2)-1)+1:g.ss_num*3+7*(g.lm_ini_pose_correspondance_3d(lm_idx,2)-1)+3) + ...
        (1/l(3))*[cos(l(2))*cos(l(1));cos(l(2))*sin(l(1));sin(l(2))];
    
    
    l_local = transform_matrix_from_trans_ypr(pose3d_quat_to_ypr(x))\[lm_3d_xy_coord;1];
