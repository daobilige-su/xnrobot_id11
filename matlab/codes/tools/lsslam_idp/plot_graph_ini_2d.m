...
if (length(l) > 0)
  ...
  lm_ini_2d_xy_coord = zeros(g.ss_num,2);
  for n=1:g.ss_num
    lm_ini_2d_xy_coord(n,:) = (g.x(g.ss_num*2+3*(g.lm_ini_pose_correspondance_2d(n,2)-1)+1:g.ss_num*2+3*(g.lm_ini_pose_correspondance_2d(n,2)-1)+2) + ...
  ...
  plot(lm_ini_2d_xy_coord(:,1), lm_ini_2d_xy_coord(:,2), 'LineStyle','none','Marker','o', 'MarkerSize', 10,'MarkerEdgeColor','b','MarkerFaceColor','y');
