function plot_initial_odom_and_landmark_3d(robot_pose_ob_3d,doa_ob_l,doa_dist_max,rblm_rb_ob_l,figure_num)
...
for n=1:size(robot_pose_ob_3d,1)
    ...
    for m = 1:size(rblm_rb_ob_l,1)
        if rblm_rb_ob_l(m,4)==n
            rblm_ob_direction_point = rblm_rb_ob_l(m,1)*[cos(rblm_rb_ob_l(m,3))*cos(rblm_rb_ob_l(m,2));cos(rblm_rb_ob_l(m,3))*sin(rblm_rb_ob_l(m,2));sin(rblm_rb_ob_l(m,3))];
            
            rblm_ob_direction_point_transformed_hom = transform_matrix_from_trans_ypr(robot_pose_ob_3d(n,:))*[rblm_ob_direction_point;1];
            rblm_ob_direction_point_transformed = rblm_ob_direction_point_transformed_hom(1:3);
            
            plot3([point_origin_transformed(1),rblm_ob_direction_point_transformed(1)],[point_origin_transformed(2),rblm_ob_direction_point_transformed(2)],...
                [point_origin_transformed(3),rblm_ob_direction_point_transformed(3)],'Color','k');
        end
    end
